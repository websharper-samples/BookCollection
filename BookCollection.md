...
![Final application](image/finaltemplate.png)
...
![Initial list](image/showonly.png)
...
![Adding items](image/removebutton.png)
...
to display the element in edit mode when needed:
	...
	Books.View.DocSeqCached((Book book) =>
	{
		if (book.IsEdited)
		{
			var editTitle = Var.Create(book.); // new Vars for editing state
			var editAuthor = Var.Create("");
			var editPublishDate = Var.Create("");
			var editISBN = Var.Create("");
...
![Adding items](image/editing.png)
	
## Summary

You can find the entire project on [GitHub]() with an enhanced template.
